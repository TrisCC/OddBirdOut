...
<section id="manipulation" class="content-section {visible ? 'visible' : ''}">
  ...
  <p class="section-intro">
    ...
    Instead, it generates a tailored reality for each player — one where the
    <em>other two</em> appear to cooperate exclusively and reject the one
    watching. Every player privately believes <strong>they</strong> are the odd
    bird out.
  </p>
...
<style>
  ...
  .scene-ostrich { width: 48px; image-rendering: pixelated; }
  .scene-ostrich--leftout { opacity: 0.4; filter: grayscale(0.5); animation: bob 2.5s ease-in-out infinite; }
  .scene-ostrich--ally { animation: bob 2s ease-in-out infinite; }
  .scene-ostrich--ally:nth-child(2) { animation-delay: 0.3s; }

  .scene-heart { width: 24px; image-rendering: pixelated; opacity: 0.7; }
